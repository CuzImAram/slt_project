...
% Read overall means: expects columns "pipeline,overall_mean_seconds"
\pgfplotstableread[col sep=comma]{data/rag_time_overall.csv}\overallmeans
% Assume row 0 = Pipeline 1, row 1 = Pipeline 4 (adjust if your file differs)
\pgfplotstablegetelem{0}{overall_mean_seconds}\of{\overallmeans}
\edef\PoneMean{\pgfplotsretval}
\pgfplotstablegetelem{1}{overall_mean_seconds}\of{\overallmeans}
...
\section{Methods}
	...
	All LLM calls in this work are performed using the Helmholtz-Blablador API~\cite{blablador_api} with the model \texttt{"alias-large"}~\cite{yang2025qwen3technicalreport}, also referred to as Qwen3-30B-A3B, optimized for information retrieval and question-answering tasks.
	...
	% (shortened a lot)
	Given that our model (\texttt{Qwen3-30B-A3B}) is an MoE (128 experts, 8 active), per-token compute is lower than dense models; prior work reports roughly 30–50\% savings, which helps amortize Pipeline~4’s additional calls~\cite{yang2025qwen3technicalreport,yun2024inferenceoptimalmixtureofexpertlargelanguage}.
...
\begin{axis}[
  ...
  xlabel={Win rate (\%)},
  xmin=0, xmax=50,
  symbolic y coords={Pipeline 1,Pipeline 4,Don't Care},
  ytick=data,
  nodes near coords,
  nodes near coords align={horizontal},
  bar width=15pt,
]
\addplot coordinates {(25.0,Pipeline 1) (43.8,Pipeline 4) (31.2,Don't Care)};
\end{axis}
\end{tikzpicture}
...
\paragraph{Knowledge override on sensitive topics.}
We observed instances where the model answered \emph{regardless} of available
context, i.e., overrides content based on own knowledge, particularly for highly sensitive topics
...
] table[x=qid, y=mean_time_seconds, col sep=comma]{data/rag_time_long.csv};
...
% --- NEW: semi-transparent mean lines ---
\addplot+[domain=1:20, samples=2, blue, densely dashed, very thick, opacity=0.35] {\PoneMean};
\addlegendentry{P1 mean}

\addplot+[domain=1:20, samples=2, red, densely dashed, very thick, opacity=0.35] {\PfourMean};
\addlegendentry{P4 mean}
...
\paragraph{Limitations.}
Three factors temper this advantage: 
\newline(i) retrieval coverage gaps where Elasticsearch returned no meaningful snippets 
\newline(ii) occasional knowledge overrides—especially on sensitive topics—where the model answered without sufficient grounding  
\newline(iii) higher LLM runtime for Pipeline~4 due to additional expansion/aggregation steps. Timing analysis shows that apparent “fast” spikes for Pipeline~4 coincide with poor retrieval (short contexts), not genuine efficiency gains.

\paragraph{Overall.}
Despite the limitations, query-pool expansion provides consistent benefits in well-grounded settings. 
Our \hyperref[performance-comparison]{results} support \hyperref[hyp:H1]{H\textsubscript{1}} and show that 
Pipeline~4 has a modest advantage, though at the cost of increased runtime. 
In resource-constrained environments, this cost–quality trade-off requires careful consideration.
